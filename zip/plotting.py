...
def get_bls_results(lc):
    ...
    stats = model.(results.period[np.argmax(results.power)], 
...
def get_bls_results_gpu(lc):
    ...
    periods = np.linspace(2.0, 30.0, 5000)  # days
    durations = np.linspace(0.1, 1.0, 1000)  # days
    freqs = 1 / periods
    
    print('Running Cuvarbase...')
    start = time.time()
    bls_power, sols = bls.eebls_gpu(lc.time.value, lc.flux.value, lc.flux_err, freqs=freqs)
    print(bls_power[np.argmax(bls_power)])
    print(f'Cuvarbase finished. Time: {time.time() - start}')
    model = BoxLeastSquares(lc.time, lc.flux)
    
    best_idx = np.argmax(bls_power)
    ...
    t_start = lc.time[0]
    t0 = t_start + (phi0_best * period * u.day)
    

    t0_start = lc.time[0].value + (phi0_best * period)  # Start of transit (in days)
    t0_mid_value = t0_start + (duration / 2)            # Mid-transit (in days)
    t0_mid = t0_mid_value * u.day                       # Add units if needed
    

    print(f"BLS t0_mid: {t0_mid_value}, Catalog t0: 1332.11425. t0_mid: {t0_mid}")
    
    stats = model.compute_stats(period, duration, t0-1.8)
    stats['period'] = period
