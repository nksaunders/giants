...
class Giant(object):
    ...
    def from_eleanor(self, save_postcard=False):
        ...
        star = eleanor.Source(tic=self.ticid, sector=int(sectors[0]), tc=False)
        try:
        ...
        if len(sectors) > 1:
            for s in sectors[1:]:
                try: # some sectors fail randomly
                    star = eleanor.Source(tic=self.ticid, sector=int(s), tc=False)
...
if __name__ == '__main__':
    try:
        target = Giant(ticid=*sys.argv[1:], csv_path='data/ticgiants_allsky_halo.csv')
        target.plot()
    except:
